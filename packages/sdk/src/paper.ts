...
export interface PaperOrder { id: string; mint: string; symbol: string; side: 'buy' | 'sell'; quantity: number; priceUsd: number; totalUsd: number; createdAt: string; }
...
export function parsePaperAccount(value: unknown): PaperAccount | null {
  ...
  if (!account.orders.every(item => { const o = record(item); return text(o.id) && text(o.mint) && text(o.symbol) && (o.side === 'buy' || o.side === 'sell') && positive(o.quantity) && positive(o.priceUsd) && positive(o.totalUsd) && date(o.createdAt); })) return null;
...
export function executePaperOrder(account: PaperAccount, asset: MarketAsset, side: 'buy' | 'sell', amountUsd: number, now = new Date().toISOString()): PaperAccount {
  ...
  const quantity = amountUsd / asset.priceUsd;
  ...
  const order: PaperOrder = { id: `${now}-${account.orders.length + 1}-${asset.mint.slice(0,6)}`, mint: asset.mint, symbol: asset.symbol, side, quantity, priceUsd: asset.priceUsd, totalUsd: amountUsd, createdAt: now };
