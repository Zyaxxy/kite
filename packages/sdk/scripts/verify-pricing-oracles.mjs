...
async function main() {
  console.log('================================================================');
  console.log('Kite SDK Pricing and Oracle Integration Verification');
  console.log('================================================================\n');

  // 1. Meteora DLMM API
  console.log('1. Testing Meteora DLMM API (https://dlmm.datapi.meteora.ag/pools)');
  const meteora = new MeteoraClient();
  try {
    const poolsRes = await meteora.searchPools({ limit: 3, sort_key: 'tvl' });
    console.log(`   [PASS] Successfully queried Meteora pools: total ${poolsRes.total} pools indexed.`);
    ...
    console.error('   [ERROR] Meteora error:', err.message);
  }

  console.log('\n----------------------------------------------------------------\n');

  // 2. Jupiter Price v3 API
  console.log('2. Testing Jupiter Price v3 API (https://api.jup.ag/price/v3?ids={mint})');
  ...
  try {
    ...
    console.log(`   [PASS] Single Token Price (SOL): $${JupiterPriceClient.formatPrice(singlePrice, 2)}`);

    const batchPrices = await jupiter.getPrices([solMint, usdcMint]);
    console.log(`   [PASS] Batch Query Prices:`);
    ...
    console.error('   [ERROR] Jupiter error:', err.message);
  }

  console.log('\n----------------------------------------------------------------\n');

  // 3. Pyth Network & xStocks Oracles
  console.log('3. Testing Pyth Network xStocks Equity Feeds & /public/oracles Route');
  ...
  try {
    ...
    if (searchRes.length > 0) {
      console.log(`   [PASS] Found Pyth feed for NVDAx: ${searchRes[0].attributes?.symbol} (${searchRes[0].attributes?.description})`);
      console.log(`      - Feed ID: ${searchRes[0].id}`);
    }

    console.log('\n   Fetching xStocks /public/oracles route...');
    const oracles = await fetchXStocksOracles();
    console.log(`   [PASS] xStocks Oracle Configuration available: ${oracles.length} registered feeds.`);
    for (const o of oracles.slice(0, 4)) {
      console.log(`      - ${o.symbol}: ${o.name} -> Feed ${o.feedId?.slice(0, 16)}...`);
    }
  } catch (err) {
    console.error('   [ERROR] Pyth/xStocks error:', err.message);
  }
